...
class Generator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            SelfAttention(ngf),  # attention layer


            spectral_norm(nn.ConvTranspose2d(ngf, nc, 4, 2, 1, bias=False)),
            # Add attention after intermediate feature maps
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            # Input: (nc) x 32 x 32
            spectral_norm(nn.Conv2d(nc, ndf, 4, 2, 1, bias=False)),  # 32→16
            nn.LeakyReLU(0.2, inplace=True),
            
            SelfAttention(ndf),  # Attention at 8x8 feature map

            spectral_norm(nn.Conv2d(ndf, ndf*2, 4, 2, 1, bias=False)), # 16→8
            nn.BatchNorm2d(ndf*2),
            nn.LeakyReLU(0.2, inplace=True),

            spectral_norm(nn.Conv2d(ndf*2, ndf*4, 4, 2, 1, bias=False)), # 8→4
            nn.BatchNorm2d(ndf*4),
            nn.LeakyReLU(0.2, inplace=True),



            spectral_norm(nn.Conv2d(ndf*4, 1, 4, 1, 0, bias=False)),  # 4→1
        )
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, num_epochs):
        for i, data in enumerate(dataloader, 0):
            ...
            # hinge: max(0, 1 - D(x))
            errD_real = torch.mean(F.relu(1. - real_logits))
            ...
            # hinge: max(0, 1 + D(G(z)))
            errD_fake = torch.mean(F.relu(1. + fake_logits))
            ...
            # forward again with D to get logits for G update
            fake_logits = netD(fake).view(-1)
            # hinge: maximize D(G(z)) → minimize -D(G(z))
            errG = -torch.mean(fake_logits)
            errG.backward()
            optimizerG.step()
